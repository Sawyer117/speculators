...
def _masked_weighted_mean(
    ...
    # Fork (#942): global token-weighted normalization when DSPARK_GLOBAL_LOSS_REDUCE=1.
    # Applies to EVERY caller of this helper -- the confidence BCE and the first-error
    # focal term as well as the draft loss -- so the terms that get summed into one
    # objective all share a denominator. Falls through to the per-rank path otherwise.
