...
def test_hidden_moe_logit_routing_uses_previous_generated_logits():
    harness = _GeneratedFeedbackHarness()
    harness.block_size = 3
    # correction_moe_logit_routing=True turns on the logit-feedback path, which reads
    # self.draft_vocab_size. The upstream harness never set it (this test fails the same
    # way on the branch it came from); 8 matches the embedding/lm_head widths below.
