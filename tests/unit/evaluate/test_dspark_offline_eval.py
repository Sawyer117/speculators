...
def test_target_logits_are_selected_in_draft_vocab_order():
    module = _load_module()
    # dspark_offline_eval binds its module-level `torch` only inside run(), so a
    # freshly exec'd module has torch=None. Bind it here (this test fails the same
    # way on the branch it came from).
    module.torch = torch
