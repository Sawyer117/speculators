...
# DeepSeek-V4 bf16 native-MTP vLLM serve on two Ascend nodes.
#
# Node 108:
#   NODE_RANK=0 bash examples/serve/dsv4_bf16_mtp_two_node.sh
#
# Node 109:
#   NODE_RANK=1 bash examples/serve/dsv4_bf16_mtp_two_node.sh
#
# This uses DeepSeek's built-in MTP layers, so no external draft checkpoint is
# passed. Start with MTP_NUM_SPEC_TOKENS=1, then raise to 2 or 3 after baseline
...
DEFAULT_VLLM_ROOT=/home/n84449292/m84379596/DFlash/vLLM_NPU/vllm
if [[ ! -d "$DEFAULT_VLLM_ROOT" ]]; then
  DEFAULT_VLLM_ROOT=/home/a00652497/2026/dflash-vllm/installation/vllm
fi
DEFAULT_VLLM_ASCEND_ROOT=/home/n84449292/m84379596/DFlash/vLLM_NPU/vllm-ascend
if [[ ! -d "$DEFAULT_VLLM_ASCEND_ROOT" ]]; then
  DEFAULT_VLLM_ASCEND_ROOT=
fi
VLLM_ROOT=${VLLM_ROOT:-$DEFAULT_VLLM_ROOT}
VLLM_ASCEND_ROOT=${VLLM_ASCEND_ROOT:-$DEFAULT_VLLM_ASCEND_ROOT}
CANN_HOME=${CANN_HOME:-}
...
HOST=${HOST:-0.0.0.0}
PORT=${PORT:-30000}
MTP_NUM_SPEC_TOKENS=${MTP_NUM_SPEC_TOKENS:-1}
SPECULATIVE_CONFIG=${SPECULATIVE_CONFIG:-"{\"num_speculative_tokens\":${MTP_NUM_SPEC_TOKENS},\"method\":\"mtp\",\"enforce_eager\":true}"}
