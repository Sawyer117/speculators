...
for _e in $(env | sed -n 's/^\(DSPARK_[A-Z_]*\)=.*/\1/p'); do
  case "$_e" in
    ...
    |DSPARK_MOE_BALANCE_TARGET\
    |DSPARK_LOG_EXPERT_LOAD|DSPARK_LOG_EXPERT_LOAD_EVERY\
    |DSPARK_TRACE|DSPARK_TRACE_SYNC|DSPARK_EP_CHECK|DSPARK_HS_DIR\
    |DSPARK_BLOCK_INPUT) ;;
...
PROV="$RUN/${TAG}_${TS}.provenance.txt"
{
  ...
  for _v in VERIFIER DATA HS_DIR ENDPOINT LR EPOCHS MAX_ANCHORS SEQLEN MASK_TOKEN BLOCK \
            ...
            DSPARK_LOG_EXPERT_LOAD DSPARK_LOG_EXPERT_LOAD_EVERY \
            DSPARK_TRACE DSPARK_TRACE_SYNC DSPARK_EP_CHECK BF16_EXPERTS HS_ON_MISSING DSPARK_BLOCK_INPUT; do
    eval "_val=\${$_v-}"
...
nohup env \
  ...
  DSPARK_TEACHER_DOUBLE_NORM="$TEACHER_DNORM" DSPARK_BLOCK_INPUT="${DSPARK_BLOCK_INPUT:-mask}" \
  DSPARK_MOE_BALANCE="${DSPARK_MOE_BALANCE:-0}" DSPARK_MOE_BALANCE_RATE="${DSPARK_MOE_BALANCE_RATE:-1e-3}" \
  DSPARK_MOE_BALANCE_TARGET="${DSPARK_MOE_BALANCE_TARGET:-0}" \
  DSPARK_LOG_EXPERT_LOAD="${DSPARK_LOG_EXPERT_LOAD:-0}" \
  DSPARK_TRACE="${DSPARK_TRACE:-0}" DSPARK_TRACE_SYNC="${DSPARK_TRACE_SYNC:-0}" DSPARK_EP_CHECK="${DSPARK_EP_CHECK:-0}" \
