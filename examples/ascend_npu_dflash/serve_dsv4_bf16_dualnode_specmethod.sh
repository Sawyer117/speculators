#!/usr/bin/env bash
# DeepSeek-V4-Flash **bf16** dual-node serve on 2× Atlas 800 A2 — MAINLINE stack variant.
#
# Sibling of serve_dsv4_bf16_dualnode.sh, which stays as-is for the vLLM 0.23.0 / CANN 9.0.0
# stack it was proven on. This copy differs in ONE line: the speculative method.
#
#   0.23.0 + vllm-ascend dspark-dsv4 : "mtp" and "dspark" resolve to the same path.
#   0.27.1 + vllm-ascend main        : `method` selects WHICH MODEL CLASS loads. `dspark` is a
#                                      first-class citizen with its own AscendDsparkProposer;
#                                      asking for "mtp" silently serves something else.
#
# Override with SPEC_METHOD= if you need the old behaviour. Confirm what actually loaded:
#   grep -m1 "Loading draft model" <engine log>     → should say method=dspark
#
# Also remember, on the mainline stack (all three cost us time on the A3 bring-up):
#   * export LD_LIBRARY_PATH="$CONDA_PREFIX/lib:$LD_LIBRARY_PATH" in the PARENT shell —
#     neither script does it for you, and without it the load dies on libstdc++.
#   * PREFETCH=0 — 0.27.1 made --safetensors-load-strategy prefetch mutually exclusive with
#     multithreaded load (they coexisted on 0.23.0).
#   * NUM_SPEC >= the draft's dspark_block_size. Block attention is NON-causal; below it there
#     is no error, only wrong numbers.
#
...
SPEC_METHOD="${SPEC_METHOD:-dspark}"   # ★ 主线上 method 决定加载哪个模型类;老栈用 mtp
NUM_SPEC="${NUM_SPEC:-5}"                 # = dspark_block_size (released DSV4 draft = 5).
DSPARK_AUX_LAYERS="${DSPARK_AUX_LAYERS:-[40,41,42]}"  # the target aux layers the draft's main_proj consumes (3 → 3*H).
                                          # Passed through the speculative_config's draft_model_config.hf_config (the
                                          # SAME path the HS_EXTRACT block below uses). The serve reads this via
                                          # _get_eagle3_aux_layers_from_config; leaving it unset → 4-layer EAGLE3
                                          # default (16384) → "expanded size (12288) must match (16384)" at draft time.
...
if [ -n "$DRAFT" ]; then
  ...
  SPEC_ARGS=( --hf-overrides "{\"dspark_target_layer_ids\":$DSPARK_AUX_LAYERS}"
              --speculative-config "{\"model\":\"$DRAFT\",\"num_speculative_tokens\":$NUM_SPEC,\"method\":\"$SPEC_METHOD\",\"draft_sample_method\":\"$DRAFT_SAMPLE_METHOD\",\"draft_model_config\":{\"hf_config\":{\"eagle_aux_hidden_state_layer_ids\":$DSPARK_AUX_LAYERS}}}" )
