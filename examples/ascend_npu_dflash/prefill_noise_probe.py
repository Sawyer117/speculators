...
def main() -> int:
    ...
    if mode == "prompt_logprobs":
        ...
        # ⚠ 2026-09-23 二次修:上一版换了条件却没换 body,于是 `mismatch 落在本底里`
        #   这个分支打出的是「那批 response 不是目标模型会产生的」—— 结论正好说反。
        #   实测 len512 那组 本底 4.74% / mismatch 4.07%,被打成了语料有问题。
        elif mm <= max(0.02, 3 * fr):
            print("     ⟹ mismatch 落在本底噪声里 —— **语料没问题**,它就是这个目标模型的")
            print("        贪心输出。历史上那个「response 段 mismatch 64%」是长 prefill 算错")
            print("        造成的假象,不是语料的问题。")
