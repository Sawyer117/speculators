#!/usr/bin/env bash
# DeepSeek-V4-Flash **bf16** TRIPLE-node serve on 3× Atlas 800 A2 (64 GB × 8 each).
# (3-node twin of serve_dsv4_bf16_dualnode.sh — SAME model/flags, one extra DP replica.)
#
# WHY 3 nodes: bf16 needs ≥2 nodes to FIT; the 3rd is a THROUGHPUT replica — DP3 raises
# HS-dump/serve throughput ~1.5× vs DP2 (cuts the training HS-straggler). Pure DP, so it
# stays clean: each node is a self-contained TP8 replica, NOT one model TP-split (no TP16).
#
# PARALLELISM: TP8 / DP3, **expert-parallel OFF** (default).
#   - Each node = 1 DP replica × TP8 = 8 cards, holding a FULL copy of the model
#     (w8a8 ≈ 35 GB/card, bf16 more). Three nodes → data_parallel_size=3 = 3 replicas.
...
# │ 3. WORKER node (rank 2, headless):    DP_START_RANK=2   bash ... worker      │
# │    Start head first; BOTH workers connect to the head's rpc port. All 3 must │
# │    rendezvous within ~5 min (DP handshake timeout). Set WORKER2_IP for f/w.  │
# └────────────────────────────────────────────────────────────────────────────┘
#
...
ROLE="${1:?usage: bash serve_dsv4_bf16_triplenode.sh <firewall|head|worker>   (2nd worker: DP_START_RANK=2)}"
...
WORKER2_IP="${WORKER2_IP:-}"              # rank-2 / 3rd node (set for DP=3; empty = 2-node)
...
TP="${TP:-8}"
DP="${DP:-3}"                              # 3 nodes = 3 DP replicas (dualnode default is 2)
MAXLEN="${MAXLEN:-8192}"
...
if [ "$ROLE" = "firewall" ]; then
  ...
  [ -n "$WORKER2_IP" ] && firewall-cmd --zone=trusted --add-source="$WORKER2_IP/32" --permanent  # 3rd node (DP=3)
  firewall-cmd --reload
...
if [ "$ROLE" = "worker" ]; then
  ...
  # DP_START_RANK = this worker's DP rank (1 for the 2nd node; set 2,3,… for a 3rd,4th node so DP=3+ works).
  echo ">>> [worker/rank${DP_START_RANK:-1}] starting headless engine → $LOG  (connects to head $HEAD_IP:$DP_RPC_PORT)"
  nohup vllm serve "${COMMON[@]}" --headless --data-parallel-start-rank "${DP_START_RANK:-1}" > "$LOG" 2>&1 &
  echo ">>> worker PID $!"
