...
> ⏳ 未完成。装完在此记录:`DONE.` 输出、实际版本号、以及 9.2.0-beta1 上是否需要额外处置。
...
| # | 问题 | 状态 |
|---|---|---|
| 1 | **CANN 9.2.0-beta1 + torch-npu 2.10.0.post4 能否编过 V4/SAS 算子** | ⏳ **本次唯一的新变量**。挂了的处置顺序:① `pip index versions torch-npu` 找配 9.2 的 `post5`;② 贴编译日志定位是否工具链挪位;③ 回退 CANN 9.1.0.0627(136 上验过) |
