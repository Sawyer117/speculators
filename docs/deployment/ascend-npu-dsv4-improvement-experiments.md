...
## 19. ★ 三臂结案:效果都是真的,都太小,而且都会被追平(2026-08-24)

CONV 跑到 81456 步(SELECT 的三倍)。连同 SELECT(26320)、CORRECTION(38931)与
共同基线 `faithful_ep_20260804_165215`(124479 步,全程覆盖)一起判读。

### 19.1 先修尺子:独立带根本分辨不了我们关心的区间

原来的判据是两条独立曲线的误差带合成,**±0.08**。而 conc1 的回本线是 **+0.03~0.27**(§12.1)。
⟹ **「没效果」和「有效果但看不见」在这套测量下是同一个输出。**
三条架构上毫不相关的臂全部读作"无法区分",且两两之差小到 0.01σ —— 这种一致性本身
就该让人怀疑测量而不是被测物。

修法在 §16.2 记过而一直没做:**三条 run 同种子同数据,同一步看的是同一个 batch。**
那个 batch 让 accept_len 抖多少,对两条 run 是一样的,**逐步相减就抵消了**。
合成两个独立带问的是"每条曲线抖多少",该问的是"它们的差抖多少"。

```
独立带    ±0.075 ~ ±0.082
配对差    ±0.0049 ~ ±0.0055        ⟹ 窄 15~16 倍
```

⚠ 合成数据双向验证过:真增益 +0.03 → 配对读 +0.0304±0.0071 判「真有差异」
(独立带读 +0.020±0.019,含糊);**真增益 0 → 配对读 +0.0004±0.0071 判「无法区分」**。
阴性对照比窄带子更重要 —— 会撒谎的窄尺子比宽尺子更糟。

★ **随之而来的新陷阱:带子窄到 ±0.005,几乎什么都"显著"。**
把「统计显著」读成「可以上线」正是更好的仪器制造出来的错误。判读因此改为同时报量级,
并对 <0.03 的显式标注「别据此上线」。

### 19.2 ★ 截齐步数之前,三臂排序是假的

`--max-step` 在 lead-scan 模式下**被静默忽略**(`_lead_scan` 没接参数,`main` 没传)。
这是本项目**第三个"照收不误、什么也不做"的旗标**(前两个:`--markov-head-type` 在
`--from-pretrained` 下被吞、`SELECT_RANK` 没透传白跑一轮)。**这类缺陷最贵 ——
它产出的是看起来正常的错数字。** 已修(`e1c4489`),表头现在声明截断是否生效。

修之前 vs 修之后:

```
未截齐(各读各的步数)          截齐 @26320
CORRECTION @38931  +0.0240      CONV        +0.0240 ± 0.0049
SELECT     @26320  +0.0160      SELECT      +0.0160 ± 0.0055
CONV       @81737  +0.0140      CORRECTION  +0.0130 ± 0.0050
```

**排序完全翻转。** 因为增益随训练衰减,读得越晚越小 —— 未截齐时比的其实是"谁跑得短"。

### 19.3 结论

```
            早段      @26320     @81737    形状
CONV       −0.0120   +0.0240    +0.0140   ★ 起步为负 → 爬升 → 终究被追平(峰值更晚)
SELECT     +0.1070   +0.0160      —       单调衰减,掉 85%
CORRECTION +0.0455   +0.0130      —       单调衰减,掉 71%
```

**三个机制的效果都是真的(远超配对带),都远低于回本线,而且都在被基线追平。**
这比"没效果"信息量大得多:我们加的容量**确实在起作用**,只是
**学到的东西基线迟早也会学到,我们只是让它早到一点。**

★ **CONV 起步为负**,与 200 步时 `loss +0.248` 那条一直没结的线索对上了:
25.3 M 参数要先学会,才谈得上回报 —— 先付容量的代价,后拿容量的好处。
⟹ §17.3 那条"2500 步不收拢就回头查 `place` 的 `post` 缩放"**可以结案了:不是插入点的问题,
是新参数的学习代价。**

**三条臂的经济账不同,不能一刀切:**
```
SELECT / markov   每位置多读 66.2 MB 码本   回本线 +0.03~0.27   ⟹ +0.016 远不够,否
CORRECTION        块后一次,非逐位置         成本低              ⟹ +0.013,不值
CONV              块并行前向内,零串行成本   门槛≈0(约 1% 步时) ⟹ +0.024 严格说是净赚
```
⟹ **不为 CONV 单开一条线,但块宽 16 那次重训带上它是合理的** ——
官方说它治块尾衰减,而 K=16 正是块尾最长的场景(§14.1)。

### 19.4 ★ 方向判断:瓶颈不在草稿的容量

三个**架构上毫不相关**的"加容量"都只买到 0.013~0.024 且随训练消失,
而基线自己从 2.909(5k)爬到 3.874(80k)、到 124k 步仍未收敛。

⟹ **限制我们的不是草稿的表征/打分容量,是数据与优化。** 该做的是:

```
#865 的 loss-mask 偏移        correctness 修复,不是加容量;memory 记着「折进下次重训」
更多数据 / 更多步              基线到 124k 仍在涨
块宽 16                       换目标,不是加容量(且 CONV 在那里才有主场)
```

⚠ 全部仍是**训练侧 soft accept_len** 的代理。裁决只能是转换后的服务端评测 ——
而现在正好有一台干净的 w8a8 服务机(§18)可以做这件事。
